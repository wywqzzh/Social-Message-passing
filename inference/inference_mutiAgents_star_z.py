...
if __name__ == "__main__":
    ...
    measurement_noise = [11, 1, 4, 8, 10]  # observation的noise不同人有不同的值
    ...
    action2 = np.zeros((args.n_nodes, T))  # action
    action3 = np.zeros((args.n_nodes, T))  # action
    ...
    Lambda2 = [[] for i in range(args.n_nodes)]
    Lambda3 = [[] for i in range(args.n_nodes)]
    Gamma = [[] for i in range(args.n_nodes)]
    Gamma2 = [[] for i in range(args.n_nodes)]
    Gamma3 = [[] for i in range(args.n_nodes)]
    measurement_noise_cov = [[] for i in range(args.n_nodes)]
    w = [[] for i in range(args.n_nodes)]
    w2 = [[] for i in range(args.n_nodes)]
    w3 = [[] for i in range(args.n_nodes)]
        ...
        for idx, sub_graph_res in enumerate(list_of_res):
            if id_time % args.iteration_num == 0:
                if id_time == 0:
                    ...
                    w2[idx].append(np.array([1] + [0] * (len(nodes_list[idx]) - 1)))
                    w3[idx].append(np.array([1] + [0] * (len(nodes_list[idx]) - 1)))
                    ...
                    if id_time / args.iteration_num < 2:
                        ...
                        Gamma2[idx] = (1 - alpha_gamma) * Gamma2[idx] + alpha_gamma * np.diag(temp_Gamma2)
                    Gamma3[idx] = np.eye(len(nodes_list[idx]))
                    temp_lambda = np.linalg.inv(Gamma[idx])
                    temp_lambda2 = np.linalg.inv(Gamma2[idx])
                    temp_lambda3 = np.linalg.inv(Gamma3[idx])
                    Lambda[idx].append(np.linalg.inv(Gamma[idx]))
                    Lambda2[idx].append(np.linalg.inv(Gamma2[idx]))
                    Lambda3[idx].append(np.linalg.inv(Gamma3[idx]))
                    ...
                    temp_w = np.matmul(temp_lambda2, np.ones(neighbor_num).reshape(-1, 1)) / np.matmul(
                        np.ones(neighbor_num).reshape(1, -1),
                        np.matmul(temp_lambda2, np.ones(neighbor_num).reshape(-1, 1))).reshape(1, -1)
                    temp_w = np.squeeze(temp_w, axis=1)
                    w2[idx].append(temp_w)

                    temp_w = np.matmul(temp_lambda3, np.ones(neighbor_num).reshape(-1, 1)) / np.matmul(
                        np.ones(neighbor_num).reshape(1, -1),
                        np.matmul(temp_lambda3, np.ones(neighbor_num).reshape(-1, 1))).reshape(1, -1)
                    temp_w = np.squeeze(temp_w, axis=1)
                    w3[idx].append(temp_w)

                posteriors[idx] = copy.deepcopy(gaussian(initial_mu, measurement_noise_cov[idx][-1]))
                action[idx, id_time] = action_[idx, id_time] = action2[idx, id_time] = action3[
                    idx, id_time] = initial_action
            ...
            if id_time % args.iteration_num != 0:
                ...
                action2[idx, id_time] = np.dot(w2[idx][-1], np.array(temp_a))
                action3[idx, id_time] = np.dot(w3[idx][-1], np.array(temp_a))
            else:
    ...
    result_dict = {"z": z_list, "x": x, "mean": mean_, "cov": cov_, "simga": measurement_noise_cov,
                   ...
                   "nodes_list": nodes_list, "true_sigma": measurement_noise, "action2": action2, "action3": action3,
                   "w2": w2, "w3": w3}
    with open("data/neighbor_star_z_differentNoise_uniform2(" + str(args.iteration_num) + "_" + str(
